    ...
    public byte[] GenerateEmployeeListPdf(List<EmployeeDto> employees)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Text("Employee List Report").SemiBold().FontSize(20).FontColor(Colors.Blue.Medium);
    ...
    public byte[] GenerateEmployeeCvPdf(EmployeeDto employee)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Text($"Employee CV - {employee.Name}").SemiBold().FontSize(20).FontColor(Colors.Blue.Medium);
